function [hHighRes] = trainLSFilter(numIter,wsize)
...
% Process each of the 24 Kodak images
...
for i = 1:24
    ...
    X = rgb2gray(double(imread(sprintf('kodim%02d.png',i))) / 255);
    ...
    BH1 = BH1 + FH1(:,dir) * Y(:,dir)';
    BH2 = BH2 + FH2(:,dir) * Y(:,dir)';
    BH3 = BH3 + FH3(:,dir) * Y(:,dir)';
    BH4 = BH4 + FH4(:,dir) * Y(:,dir)';
    
    BV1 = BV1 + FH1(:,~dir) * Y(:,~dir)';
    BV2 = BV2 + FH2(:,~dir) * Y(:,~dir)';
    BV3 = BV3 + FH3(:,~dir) * Y(:,~dir)';
    BV4 = BV4 + FH4(:,~dir) * Y(:,~dir)';
end
...
for n = 1:numIter
    ...
    for i = 1:24
        ...
        BH1 = BH1 + FH1(:,dir) * X(:,dir)';
        BH2 = BH2 + FH2(:,dir) * X(:,dir)';
        BH3 = BH3 + FH3(:,dir) * X(:,dir)';
        BH4 = BH4 + FH4(:,dir) * X(:,dir)';

        BV1 = BV1 + FV1(:,~dir) * X(:,~dir)';
        BV2 = BV2 + FV2(:,~dir) * X(:,~dir)';
        BV3 = BV3 + FV3(:,~dir) * X(:,~dir)';
        BV4 = BV4 + FV4(:,~dir) * X(:,~dir)';
    end
    ...
    for i = 1:24
        ...
        BV1 = BV1 + FV1(:,~dir) * Y(:,~dir)';
        BV2 = BV2 + FV2(:,~dir) * Y(:,~dir)';
        BV3 = BV3 + FV3(:,~dir) * Y(:,~dir)';
        BV4 = BV4 + FV4(:,~dir) * Y(:,~dir)';
    end
